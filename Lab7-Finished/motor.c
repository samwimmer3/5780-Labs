...
void PI_update(void) {
    ...
    /// TODO: calculate error signal and write to "error" variable
		...
		// 2:1
		// RPM is 0.5 encoder counts
		// interrupt period = 37.5 ms
		// 37500 timer ticks
		// PSC = 15;
		// ARR = timer tickers / 2 = 18750
    ...
    error_integral = error_integral + (duty_cycle * error);
		
		
    /// TODO: Clamp the value of the integral to a limited positive range
    if(error_integral > 3200)
		{
			error_integral = 3200; // may need to change this
		}
		else if(error_integral < 0)
		{
			error_integral = 0; // may need to change this
		}
